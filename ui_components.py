"""
UI Components for CT Review Tool - Interface Only
"""
...
from core_functionality import (
    ReviewSession, analyze_section_with_ai, process_chat_query,
    create_reviewed_document_with_proper_comments, extract_document_sections_from_docx,
    HAWKEYE_SECTIONS, EXCLUDED_SECTIONS, accepted_feedback, rejected_feedback, 
    user_feedback, current_section_feedback, current_session, review_completed
)
...
class InteractiveReviewUI:
    ...
    def create_ui(self):
        ...
        self.prev_btn = widgets.Button(
            description='← Previous', disabled=True, button_style='primary'
        )
        self.next_btn = widgets.Button(
            description='Next →', disabled=True, button_style='primary'
        )
        ...
        self.right_panel_tabs.set_title(0, '📝 Feedback')
        self.right_panel_tabs.set_title(1, '💬 Chat')
            ...
            <div id="chat-messages" style="height: 400px; overflow-y: auto; padding: 10px; 
                ...
                <div style="padding: 10px;">
                    <div style="background: #e3f2fd; padding: 10px; border-radius: 8px; margin: 5px 0;">
                        ...
                        Hello! I'm here to help you with the document review.
                    </div>
                </div>
            </div>
        """)
    ...
    def initialize_document(self, sections, section_paragraphs, paragraph_indices, doc_object):
        ...
        with self.status:
            ...
            print(f"✅ Loaded document with {len(self.section_names)} sections")
            ...
            for i, item in enumerate(feedback_items):
                ...
                def make_accept_handler(idx, section, feedback_item, reject_button, status):
                    def handler(b):
                        ...
                        status.value = "<span style='color: #2ecc71;'>✓ Accepted</span>"
                        self._update_stats()
                    return handler
                ...
                def make_reject_handler(idx, section, feedback_item, accept_button, status):
                    def handler(b):
                        ...
                        status.value = "<span style='color: #e74c3c;'>✗ Rejected</span>"
                        self._update_stats()
                    return handler
    ...
    def _update_risk_indicator(self, feedback_items):
        ...
        if high_risk > 0:
            color = '#e74c3c'
            text = f'⚠️ High Risk ({high_risk} issues)'
        elif medium_risk > 0:
            color = '#f39c12'
            text = f'⚠️ Medium Risk ({medium_risk} issues)'
        else:
            color = '#2ecc71'
            text = '✓ Low Risk'
    ...
    def _accept_feedback(self, idx, section, item):
        ...
        with self.status:
            print(f"✓ Accepted feedback #{idx+1} for {section} - Will be added as comment")
    ...
    def _reject_feedback(self, idx, section, item):
        ...
        with self.status:
            print(f"✗ Rejected feedback #{idx+1} for {section}")
    ...
    def _add_custom_feedback(self, b):
        ...
        with self.status:
            print(f"✓ Added custom feedback for {section}")

        self._update_stats()

        all_feedback = self.feedback_data.get(section, []) + [feedback]
        self.feedback_data[section] = all_feedback
    ...
    def _complete_review(self, b):
        ...
        global review_completed, current_session
            ...
            if os.path.exists(file_path):
                ...
                if output_path and os.path.exists(output_path):
                    with self.status:
                        ...
                        display(FileLink(output_path, result_html_prefix="📄 Download: "))
        ...
        if file_upload.value:
            ...
            try:
                ...
                with status_output:
                    ...
                    print(f"📊 Found {len(sections)} sections for review")
